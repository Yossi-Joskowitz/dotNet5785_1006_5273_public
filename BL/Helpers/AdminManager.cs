...
internal static class AdminManager //stage 4
{
    ...
    internal static void UpdateClock(DateTime newClock) 
    {
        ...
        if (_periodicTask is null || _periodicTask.IsCompleted) 
            _periodicTask = Task.Run(() => Tools.UpdateExpired()); 
    ...
    internal static void Stop()
    {
        if (s_thread is not null)
        {
            ...
            s_thread.Join(); //wait for the thread to finish
    ...
    private static void clockRunner()
    {
        while (!s_stop)
        {
            ...
            if (_simulateTask is null || _simulateTask.IsCompleted)//stage 7
                _simulateTask = Task.Run(() => VolunteerManager.Simulation());          
    ...
    internal static void IsSimulationRunning()
    {
        while( (_simulateTask != null && !_simulateTask.IsCompleted) || (_periodicTask != null && !_periodicTask.IsCompleted))
        {
            Thread.Sleep(500);
        }
        Thread.Sleep(1000);
